...
export const CalendarType: React.FunctionComponent<{item: IAppView, onChange: (event, config) => void}> = props => {

    if (props == null || props.item == null || typeof props.onChange !== 'function') return null;

    let config = props.item.config || {};

    if (config._calendar == null) {
        config._calendar = { };
    ...
    return (
        <div>
            ...
            <Stack tokens={{ padding: 10 }}>
                <h3 className="panel-header">Calendar Properties</h3>
            </Stack>
            <div className={separator}></div>
        </div>
    );
